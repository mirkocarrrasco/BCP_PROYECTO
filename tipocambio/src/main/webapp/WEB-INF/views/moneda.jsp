	...
	<% if (request.getAttribute("lista") != null) {	%>
		<div class="table-responsive">
			<table class="table table-responsive table-sm table-light table-striped table-bordered table-hover">
				...
				<tbody>
					<%
						List<MonedaEntity> lista = (List<MonedaEntity>) request.getAttribute("lista");
						for (MonedaEntity monedaEntity : lista) {
					%>
						<tr>
							<td><%= monedaEntity.getIdmoneda() %></td>
							<td><%= monedaEntity.getNommoneda() %></td>
							<td>
								<a href="monedaEliminar?codigo=<%= monedaEntity.getIdmoneda() %>">SI</a>
							</td>
						</tr>
					<% } %>
				</tbody>
